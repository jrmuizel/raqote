    ...
    match src {
        ...
        Source::Image(ref image, ExtendMode::Pad, filter, transform) => {
            if let Some(offset) = is_integer_transform(&ti.post_transform(&transform)) {
                *shader_storage = ShaderStorage::ImagePadAlpha(ImagePadAlphaShader::new(
                ...
                if alpha != 255 {
                    ...
                    if *filter == FilterMode::Bilinear {
                        let s = TransformedImageShader::<PadFetch>::new(
                            image,
                            &ti.post_transform(&transform),
                        );
        ...
        Source::Image(ref image, ExtendMode::Repeat, filter, transform) => {
            if let Some(offset) = is_integer_transform(&ti.post_transform(&transform)) {
                *shader_storage = ShaderStorage::ImageRepeatAlpha(ImageRepeatAlphaShader::new(
                    image, offset.x, offset.y, alpha,
                ));
                shader = match shader_storage {
                ...
                if *filter == FilterMode::Bilinear {
                    if alpha != 255 {
                        ...
                        *shader_storage = ShaderStorage::TransformedRepeatImageAlpha(s);
                        ...
                        let s = TransformedImageShader::<RepeatFetch>::new(
                            image,
                            &ti.post_transform(&transform),
                        );
        ...
        Source::RadialGradient(ref gradient, spread, transform) => {
            let s =
                RadialGradientShader::new(gradient, &ti.post_transform(&transform), *spread, alpha);
        ...
        Source::LinearGradient(ref gradient, spread, transform) => {
            let s =
                LinearGradientShader::new(gradient, &ti.post_transform(&transform), *spread, alpha);
